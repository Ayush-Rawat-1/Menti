...
export default function MessageBubble({ message, isStreaming }: Props) {
  ...
  return (
    <div className={`flex items-end gap-3 animate-slide-up ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
      {/* Avatar */}
      {!isUser && (
        <div className="flex-shrink-0 w-8 h-8 rounded-full bg-forest-600 flex items-center justify-center mb-0.5">
        ...
        <div
          className={`
            ...
            ${isUser
              ...
              : 'bg-white border border-cream-200 text-stone-850 rounded-bl-sm shadow-soft'
            }
          ...
          {isStreaming && (
            <span className="inline-flex items-center gap-0.5 ml-1">
              {[0, 150, 300].map((delay) => (
                <span
                  key={delay}
                  className="w-1 h-1 rounded-full bg-current opacity-60 animate-pulse-dot"
